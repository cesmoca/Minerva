...
| 5 | Core MAO objects: `MAO` is active; `MAOPositionator3D`, `MAOMark`, and `MAOMarksGroup` remain | Resources, value/property types, Python binding foundation | No new dependency beyond the active OpenCV and Boost.Python foundations |
...
Phases 2 through 4 are complete, and `MAO` is active in phase 5.
`MAOPositionator3D` is the next dependency-ordered phase-5 unit.
